...
class LoanRulesEditor extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.policies && !isEqual(nextProps.policies, this.props.policies)) {
    ...
    if (nextProps.code !== this.props.code) {
      this.setState(prevState => Object.assign(_.cloneDeep(prevState), { code: nextProps.code }));
    }
  ...
  showHelp(cm) {
    ...
    const hintOptions = {
      ...
      customKeys: {
        'Up': moveFocusUp,
        'Down': moveFocusDown,
        'PageUp': (cmInstance, handle) => { handle.moveFocus(-handle.menuSize() + 1, true); },
        'PageDown': (cmInstance, handle) => { handle.moveFocus(handle.menuSize() - 1, true); },
        'Home': (cmInstance, handle) => { handle.setFocus(0); },
        'End': (cmInstance, handle) => { handle.setFocus(handle.length - 1); },
        'Enter': handleEnter,
        'Tab': handleTab,
        'Esc': (cmInstance, handle) => { handle.close(); },
      }
    };
